...
## Autonomous Delivery Standard (DocuMint)

When the user requests execution (e.g., "run it", "continue", "ship it"):

1. Run in autonomous batches: Audit → Plan → Implement → Verify → Document
2. Do not wait for repeated user prompts between batches unless blocked
3. End each batch with:
   - changed files
   - verification status
   - risk notes
   - next batch plan
